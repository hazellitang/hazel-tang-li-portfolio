...
function FilmProjector({ lang, onOpen }: { lang: Lang; onOpen: (project: Project) => void }) {
  ...
  const left = films.filter((_, index) => index % 2 === 0)
  const right = films.filter((_, index) => index % 2 !== 0)

  return (
    <div className="projector reveal">
      <div className="project-index project-index--left">
        {left.map((project) => <FilmIndexButton key={project.id} project={project} active={active.id === project.id} lang={lang} onClick={() => setActive(project)} />)}
      </div>
      ...
      <div className="project-index project-index--right">
        {right.map((project) => <FilmIndexButton key={project.id} project={project} active={active.id === project.id} lang={lang} onClick={() => setActive(project)} />)}
      </div>
      ...
      <article className={`project-drawer project-drawer--${project.kind}`}>
        ...
        {project.gallery?.length ? (
          <section className="project-gallery">
            <div className="gallery-heading"><span>{labels.bts[lang]}</span><b>{String(project.gallery.length).padStart(2, '0')} IMAGES</b></div>
            <div>{project.gallery.map((item, index) => <figure key={item.src}><img src={item.src} alt={item.alt[lang]} /><figcaption>{String(index + 1).padStart(2, '0')} · {item.type === 'bts' ? labels.bts[lang] : lang === 'en' ? 'Film still' : '影片静帧'}</figcaption></figure>)}</div>
          </section>
        ) : (
  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        <div className="hero-ambient" aria-label="Abstract moving-image background"><i className="ambient-orbit ambient-orbit--one" /><i className="ambient-orbit ambient-orbit--two" /><i className="ambient-gate" /><span>MOVING IMAGE / PRODUCTION / STORY</span></div>
        <div className="hero-scrim"/>
        <div className="hero-number">01 — 07</div>
      ...
      <section className="profile section" id="about">
        <div className="section-marker">02 / ABOUT ME</div>
        ...
        <div className="profile-layout reveal">
          <div className="portrait-slot"><span>PORTRAIT<br/>READY TO REPLACE</span></div>
          <div className="bio">
          ...
          <div className="archive-stats"><div><b>78</b><span>{lang === 'en' ? 'commercial & educational videos' : '商业及教育短视频'}</span></div><div><b>4M+</b><span>{lang === 'en' ? 'recorded cross-platform views' : '已记录跨平台播放'}</span></div><div><b>07</b><span>{lang === 'en' ? 'connected working roles' : '贯穿项目的工作角色'}</span></div><div><b>03</b><span>{lang === 'en' ? 'cities of practice' : '主要实践城市'}</span></div></div>
        </div>
      </section>

      <section className="commercial section" id="commercial">
        <div className="section-marker">03 / COMMERCIAL</div>
      ...
      <section className="films section" id="films">
        <div className="section-marker">04 / SCREENING ROOM</div>
      ...
      <section className="theatre-section section" id="theatre">
        <div className="section-marker">05 / STAGE DOOR</div>
      ...
      <section className="method section" id="method">
        <div className="section-marker">06 / CORE SKILLS</div>
        ...
        <div className="method-table">{abilities.map((ability) => <div className="method-row reveal" key={ability.index}><span>{ability.index}</span><h3>{ability.title[lang]}</h3><p>{ability.items[lang]}</p></div>)}</div>
      </section>

      <footer className="contact" id="contact">
        <div className="night-road"><i/><i/><i/></div><p>07 / CONTACT</p>
