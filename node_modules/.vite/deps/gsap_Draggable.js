...
var _divContainer;
var _svgContainer;
var _identityMatrix$1;
var _gEl;
var _transformProp$1 = "transform";
var _transformOriginProp$1 = _transformProp$1 + "Origin";
var _hasOffsetBug;
...
var _svgTemps = [];
var _divTemps = [];
var _getDocScrollTop$1 = function _getDocScrollTop() {
	return _win$1.pageYOffset || _doc$1.scrollTop || _docElement$1.scrollTop || _body$1.scrollTop || 0;
};
var _getDocScrollLeft$1 = function _getDocScrollLeft() {
...
var _isFixed$1 = function _isFixed(element) {
	if (_win$1.getComputedStyle(element).position === "fixed") return true;
...
var _placeSiblings = function _placeSiblings(element, adjustGOffset) {
	...
	if (element === _win$1) return element;
	...
	if (svg) {
		...
		if (_hasOffsetBug) {
			...
			b = element;
			while (b && (b = b.parentNode) && b !== m && b.parentNode) if ((_win$1.getComputedStyle(b)[_transformProp$1] + "").length > 4) {
				x = b.offsetLeft;
				y = b.offsetTop;
				b = 0;
			}
...
function getGlobalMatrix(element, inverse, adjustGOffset, includeScrollInFixed) {
	if (!element || !element.parentNode || (_doc$1 || _setDoc(element)).documentElement === element) return new Matrix2D();
...
var _bigNum = 0x56bc75e2d63100000;
...
var _temp1 = {};
var _windowProxy = {};
var _copy = function _copy(obj, factor) {
	var copy = {}, p;
	for (p in obj) copy[p] = factor ? obj[p] * factor : obj[p];
	return copy;
};
var _extend = function _extend(obj, defaults) {
	for (var p in defaults) if (!(p in obj)) obj[p] = defaults[p];
	return obj;
};
var _setTouchActionForAllDescendants = function _setTouchActionForAllDescendants(elements, value) {
...
var _renderQueueTick = function _renderQueueTick() {
	return _renderQueue.forEach(function(func) {
		return func();
	});
};
var _addToRenderQueue = function _addToRenderQueue(func) {
	_renderQueue.push(func);
	if (_renderQueue.length === 1) gsap.ticker.add(_renderQueueTick);
};
var _renderQueueTimeout = function _renderQueueTimeout() {
	return !_renderQueue.length && gsap.ticker.remove(_renderQueueTick);
};
var _removeFromRenderQueue = function _removeFromRenderQueue(func) {
...
var _addListener = function _addListener(element, type, func, capture) {
...
var _hasTouchID = function _hasTouchID(list, ID) {
	var i = list.length;
	while (i--) if (list[i].identifier === ID) return true;
};
var _onMultiTouchDocumentEnd = function _onMultiTouchDocumentEnd(event) {
	_isMultiTouching = event.touches && _dragCount < event.touches.length;
	_removeListener(event.target, "touchend", _onMultiTouchDocumentEnd);
};
var _onMultiTouchDocument = function _onMultiTouchDocument(event) {
	_isMultiTouching = event.touches && _dragCount < event.touches.length;
	_addListener(event.target, "touchend", _onMultiTouchDocumentEnd);
};
var _getDocScrollTop = function _getDocScrollTop(doc) {
	return _win.pageYOffset || doc.scrollTop || doc.documentElement.scrollTop || doc.body.scrollTop || 0;
};
var _getDocScrollLeft = function _getDocScrollLeft(doc) {
	return _win.pageXOffset || doc.scrollLeft || doc.documentElement.scrollLeft || doc.body.scrollLeft || 0;
};
var _addScrollListener = function _addScrollListener(e, callback) {
	_addListener(e, "scroll", callback);
	if (!_isRoot(e.parentNode)) _addScrollListener(e.parentNode, callback);
};
var _removeScrollListener = function _removeScrollListener(e, callback) {
	_removeListener(e, "scroll", callback);
	if (!_isRoot(e.parentNode)) _removeScrollListener(e.parentNode, callback);
};
var _isRoot = function _isRoot(e) {
	return !!(!e || e === _docElement || e.nodeType === 9 || e === _doc.body || e === _win || !e.nodeType || !e.parentNode);
};
var _getMaxScroll = function _getMaxScroll(element, axis) {
	var dim = axis === "x" ? "Width" : "Height", scroll = "scroll" + dim, client = "client" + dim;
	return Math.max(0, _isRoot(element) ? Math.max(_docElement[scroll], _body[scroll]) - (_win["inner" + dim] || _docElement[client] || _body[client]) : element[scroll] - element[client]);
};
var _recordMaxScrolls = function _recordMaxScrolls(e, skipCurrent) {
...
var _setStyle = function _setStyle(element, property, value) {
...
var _tempRect = {};
var _parseRect = function _parseRect(e) {
	if (e === _win) {
...
var _dispatchEvent = function _dispatchEvent(target, type, callbackName) {
...
var _getBounds = function _getBounds(target, context) {
...
var _point1 = {};
var _getElementBounds = function _getElementBounds(element, context) {
...
var _parseInertia = function _parseInertia(draggable, snap, max, min, factor, forceZeroVelocity) {
	var vars = {}, a, i, l;
	if (snap) {
...
var _isClickable = function _isClickable(element) {
	var data;
...
var _setSelectable = function _setSelectable(elements, selectable) {
...
var _supports3D;
var _addPaddingBR;
var ScrollProxy = function ScrollProxy(element, vars) {
...
var _initCore = function _initCore(required) {
